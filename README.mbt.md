...
| [`subtotals.yxl.yaml`](./examples/subtotals.yxl.yaml) | a store list with subtotals beneath it — per branch, per channel within each branch, and a grand total — as live `SUMIFS` formulas in a fixed order; a named layout another sheet sums by name, a chart placed under the list and plotting it by name, a store drop-down fed by it, and an override that finds its row by the data's values |
