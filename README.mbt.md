...
| [`columns.yxl.yaml`](./examples/columns.yxl.yaml) | a year-on-year report whose columns are named, not lettered: `layouts:` with a two-row merged header, `{{name}}` formulas, per-column conditional formatting, and rows that fill only the input columns |
